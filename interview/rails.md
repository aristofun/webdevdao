...
1. Что такое `partial` и для чего используются?
    ...
    <details>
      <summary>Ответ</summary>
      partial — это кусочек кода представления, который можно вынести в отдельный шаблон для удобства и для использования в других представлениях.
    </details>
...
1. Что такое Haml, Slim? Какие плюсы на ваш взгляд, их использования?
    ...
    <details>
      <summary>Ответ</summary>
      Haml и Slim — это шаблонизаторы, используются для удобства и минимизации написания кода в представлениях. Сокращают в несколько раз написание кода, нет проблем в закрывании тегов, не получится так, что тег не закрыт и код не работает. Меньше вероятность того, что можно ошибиться + лучше читаемость в коде.
...
1. Как реализовано кеширование в рельсах?
    ...
    <details>
      ...
      Кэширование значительно ускоряет загрузку страниц, снижает количество запросов к серверу.
...
1. Чем отличаются методы `find` и `find_by`?
    ...
    <details>
      <summary>Ответ</summary>
      Если запись не найдена, `find_by` вернет nil, а `find` - исключение ActiveRecord::RecordNotFound.
      `find` может быть использован только для поиска по первичному ключу (primary key, который обычно является _id_).
      `find_by` можно передать и провести поиск по аттрибуту.
    </details>
